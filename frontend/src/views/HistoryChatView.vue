...
import MyAgentsView from './MyAgentsView.vue'
...
// ========== state ==========
const currentPage = ref<'chat' | 'agents'>('chat')
...
// 历史对话操作
const showChatMenuId = ref<number | null>(null)
...
const renameTargetId = ref<number | null>(null)
  ...
  const deletedChatIds = ref<number[]>([])
...
function openChatMenu(id:number){
  showChatMenuId.value =
    showChatMenuId.value === id ? null : id
}


function openRename(chat:ChatItem){
...
function saveRename(){
  ...
  const chat = chats.value.find(
    item=>item.id===renameTargetId.value
  )
...
const deletedIds = JSON.parse(
  localStorage.getItem('deleted-chat-ids') || '[]'
)
...
>
  ...
  >
    ...
    <el-icon :size="16" class="mt-0.5 shrink-0">
      <ChatDotSquare />
    </el-icon>
...
<button
  class="
    ...
    group-hover:flex
  "
  @click.stop="openChatMenu(chat.id)"
>
  ⋮
</button>
...
>
    ...
    <div class="flex flex-1 flex-col min-w-0">
      <MyAgentsView v-if="currentPage === 'agents'"
